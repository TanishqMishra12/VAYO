...
class DatabaseManager:
    ...
    async def filter_communities_by_location(self, city: str, timezone: str, limit: int = 1000):
        query = """
            SELECT 
                c.community_id,
                c.community_name,
            ...
            FROM communities c
            LEFT JOIN (
                SELECT community_id, COUNT(*) as message_count
    ...
    def vector_search(self, query_vector: List[float], community_ids: List[str], top_k: int = 20):
    ...
    async def get_popular_communities(self, limit: int = 5):
        query = """
            SELECT 
                c.community_id,
                c.community_name,
    ...
    async def auto_join_community(self, user_id: str, community_id: str):
        query = """
    ...
    async def get_community_members_for_intro(self, community_id: str, limit: int = 5):
        query = """
            SELECT 
                u.user_id,
                u.username,
            ...
            LEFT JOIN community_activity ca 
